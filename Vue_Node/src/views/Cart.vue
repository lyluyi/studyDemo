            ...
            <ul class="cart-item-list">
              <li v-for="item in cartList">
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{item.productNum*parseInt(item.salePrice) | currency('$')}}</div>
                </div>
  ...
  methods:{
    init () {
      axios.get('/users/cartList').then((response) => {
        sessionStorage.setItem('bbb', '123')
        this.setCookie('aaa', '123')
    ...
    delCart () {
      /*
        '/users/cart/del' 这种请求相当于是三级路由 对于服务端而言

      而 config index.js 中的本地代理 /users/* 这种写法相当于是只能匹配到任意的二级路由
      {
        '/users/*': { // app.js 配置的users下面的所有子路由
          target: 'http://localhost:3000'
      }

      因此 '/users/**' 这种写法 就可以匹配到三级的路由
    ...
    editCart (flag, item) {
      ...
      axios.post('/users/cartEdit', {
        ...
        let res = response.data
        let num = 0
        if (flag === 'add') { // edit时 add minu时 Vuex维护购物车数量的处理
          num = 1
        } else if (flag === 'minu') {
          num = -1
        }
